    ...
    FunSpec({
        test("confirm smartphone revoke in Self Service Mode") {
            ...
            runTest {
                ...
                harness.respondTo(Topics.Command.CONFIRM_SMARTPHONE_REVOKE) {
                    val apiEvent =
                        ApiEvent(
                    ...
                    emitMessage(Topics.Event.SMARTPHONE_REVOKE_CONFIRMED, encodeEvent(apiEvent))
                }

                val api = harness.api
                val result = api.confirmSmartphoneRevokeAsync(smartphoneId, transactionId).await()

                result.mediumId.shouldBeEqual(smartphoneId)
                result.transactionId.shouldBeEqual(transactionId)

                harness
                    .publishedPayload(Topics.Command.CONFIRM_SMARTPHONE_REVOKE)
                    .shouldBeEqual(
