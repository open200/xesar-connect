...
class DeleteAuthorizationTimeProfileTest :
    FunSpec({
        test("create authorization time profile") {
            runTest {
                val harness = MockedXesarConnect(this)
                harness.respondTo(Topics.Command.DELETE_AUTHORIZATION_TIME_PROFILE) {
                    val apiEvent =
                        ApiEvent(
                ...
                harness
                    .publishedPayload(Topics.Command.DELETE_AUTHORIZATION_TIME_PROFILE)
                    .shouldBeEqual(
