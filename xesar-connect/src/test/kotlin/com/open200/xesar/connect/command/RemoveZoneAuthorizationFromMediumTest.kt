...
class RemoveZoneAuthorizationFromMediumTest :
    FunSpec({
        test("remove zone authorization from medium") {
            runTest {
                val harness = MockedXesarConnect(this)
                harness.respondTo(Topics.Command.REMOVE_ZONE_AUTHORIZATION_FROM_MEDIUM) {
                    val apiEvent =
                        ApiEvent(
                ...
                harness
                    .publishedPayload(Topics.Command.REMOVE_ZONE_AUTHORIZATION_FROM_MEDIUM)
                    .shouldBeEqual(
